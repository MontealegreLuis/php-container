...
install:
	...
	@chmod a+x config/group.sh
	@source .env.sh; rm -f config/group.sh; CONTAINER_VARS='$$HOST_USER_ID:$$HOST_GROUP_ID:$$HOST_USER'; envsubst "$$CONTAINER_VARS" < "config/group.sh.template" > "config/group.sh";
